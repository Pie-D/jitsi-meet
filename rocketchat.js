...
async function fetchWithAuth(url) {
    try {
        ...
        CONFERENCE_INFO.store.dispatch({
...
/**
 * Lấy giá trị không rỗng từ các giá trị truyền vào
 */
...
function formatMessage(msg) {
    ...
    const isLocalMessage
        = sender.username === CONFERENCE_INFO.localParticipantName
...
export async function syncRocketChatMessages(offset = 0) {
    try {
        ...
        CONFERENCE_INFO.store.dispatch({ type: PREPEND_MESSAGES,
            messages: sortedMessages });

        CONFERENCE_INFO.store.dispatch({ type: SET_HISTORY_LOADED,
            isHistoryLoaded: true });
...
function setupRocketChatWebSocket() {
    ...
    ws.onopen = () => {
        ...
        ws.send(JSON.stringify({ msg: 'connect',
            version: '1',
            support: [ '1' ] }));
        ws.send(
            JSON.stringify({
